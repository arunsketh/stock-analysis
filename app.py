...
def perform_analysis(symbols):
    ...
    # Use st.progress for better user feedback during data fetching
    ...
    # --- Create Rankings ---
    # Rank within the entire dataset before splitting
...
def display_styled_table(df):
    """Takes a DataFrame and displays it with custom styling and increased height."""
    ...
    styler = df[column_order].style.format(format_rules)
    ...
    # Calculate dynamic height: (number of rows + 1 for header) * 35 pixels per row
...
def main():
    ...
    # Default stocks include a mix of US and International
    ...
    if st.button("Get Stock Analysis"):
        ...
        if symbols_to_analyze:
            ...
            if analysis_df is not None and not analysis_df.empty:
                ...
                # Non-US stocks typically have a '.' in their symbol (e.g., 'M&M.NS')
