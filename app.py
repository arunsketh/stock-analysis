...
DEFAULT_STOCKS_FILE = "stockSs.txt"
...
def get_stock_data(symbol: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # --- 52-Week Up/Down Calculation ---
        hist = ticker.history(period="1y", interval="1wk")
        if not hist.empty:
            up_weeks = int((hist['Close'] > hist['Open']).sum())
            total_weeks = len(hist)
            down_weeks = total_weeks - up_weeks
            up_ratio = up_weeks / total_weeks
            updown_label = f"{up_weeks}↑ / {down_weeks}↓"
        else:
            up_ratio = None
            updown_label = None
        ...
        return {
            ...
            "Up Weeks %": up_ratio,       # e.g. 0.4231  → displayed as 42.3%
            "Up/Down (52W)": updown_label, # e.g. "22↑ / 30↓"
        }
...
def display_styled_table(df: pd.DataFrame):
    ...
    column_order = [
        ...
        "Up Weeks %", "Up/Down (52W)",   # ← new columns
    ]
    ...
    styler.background_gradient(cmap='RdYlGn', subset=['Upside', 'EPS', 'Up Weeks %'])
...
def render_metrics_explanation():
    ...
    with st.expander("🎓 Learn About the Metrics Used for Ranking"):
        ...
        ### 4. Up Weeks % (52-Week)
        - **Why it matters:** Shows the percentage of weeks in the past year where the stock closed higher than it opened. A higher value means the stock trended upward more consistently.
        - **Formula:** $\text{Up Weeks \\%} = \frac{\text{Weeks where Close} > \text{Open}}{52}$
        - The **Up/Down (52W)** column shows the raw count (e.g. 28↑ / 24↓) for quick reference.
        """)
